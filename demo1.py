...
def run_edge_case_tests():
    ...
    system = st.session_state.security_system
    system["sequence_analyzer"] = CommandSequenceAnalyzer()
    system["rate_limiter"] = RateLimiter()
...
def load_and_preprocess_data():
    ...
    eeg_data = df.iloc[:, 4:26].values.T
...
def extract_features(epochs, sfreq=250):
    ...
    for i in range(n_epochs):
        ...
        for ch in range(n_channels):
            sig = data[i, ch, :]
            time_feats = [np.mean(sig), np.std(sig), np.var(sig), np.max(sig) - np.min(sig), np.median(sig),
                          np.mean(np.abs(sig))]
        ...
        f_list.append(f)
        Pxx_list.append(Pxx)
    return np.array(features), f_list, Pxx_list
...
def train_or_load_model(features, raw_labels, n_epochs_actual):
    ...
    try:
        ...
        model = RandomForestClassifier(
            n_estimators=300,  # 增加决策树数量，提高稳定性
            max_depth=20,  # 增加深度，捕捉更复杂模式
            min_samples_split=5,  # 分裂节点所需最小样本数，防止过拟合
            min_samples_leaf=2,  # 叶子节点最小样本数，防止过拟合
        ...
        # 显示在侧边栏
        st.sidebar.markdown("---")
        st.sidebar.subheader("📊 交叉验证结果")
        st.sidebar.metric("平均准确率", f"{cv_scores.mean():.2%}", delta=f"±{cv_scores.std():.2%}")
        st.sidebar.metric("测试集准确率", f"{test_acc:.2%}")

        # 生成报告文件
        report_content = f"""# 脑电意图识别模型评估报告
        ...
        ## 性能指标
        ### 交叉验证（5折）
        - 平均准确率: {cv_scores.mean():.2%}
        - 标准差: {cv_scores.std():.2%}
        - 各折准确率: {', '.join([f'{s:.2%}' for s in cv_scores])}
...
# ===================== 可视化 =====================
def plot_eeg_waveform(epochs, trial_idx, ch_idx=0):
    data = epochs.get_data()[trial_idx, ch_idx, :]
    sfreq = epochs.info['sfreq']
    time = np.linspace(0, len(data) / sfreq, len(data))

    fig, ax = plt.subplots(figsize=(10, 3))
    ax.plot(time, data, color='#1890ff', linewidth=1.5)
    ax.set_title(f'第{trial_idx + 1}试次 - 第{ch_idx + 1}通道脑电信号波形', fontsize=12, fontweight='bold')
...
def plot_confidence_bar(pred_proba, le):
    ...
    ax.set_title('各类运动意图识别置信度', fontsize=12, fontweight='bold')
    ax.set_ylim(0, 100)
    ax.set_facecolor('#f8f9fa')
...
def plot_freq_spectrum(f, Pxx):
    ...
    ax.set_title('脑电信号频域功率谱', fontsize=12, fontweight='bold')
...
def create_enhanced_security_dashboard():
    ...
    <div style="padding:12px;border-radius:8px;background-color:#fffaf0;">
        ...
        <div style="margin-top:8px;">
            <div style="height:10px;background-color:#e0e0e0;border-radius:5px;overflow:hidden;">
                <div style="height:100%;width:{risk_score * 10}%;background-color:{risk_color};"></div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        if st.button("📊 安全报告", use_container_width=True):
...
def main():
    ...
    with col2:
        ...
        plot_eeg_waveform(epochs, trial_idx)
        plot_freq_spectrum(f_list[trial_idx], Pxx_list[trial_idx])
    ...
    if 识别_btn:
        ...
        with result_container:
            ...
            # 显示识别结果卡片
            st.markdown(f"""
            <div style='padding:20px; border-radius:10px; background:linear-gradient(135deg,#e6f7ff,#f0fff4); text-align:center;'>
                <h3 style='color:#006959;'>✅ 识别完成</h3>
                <h2>{pred_label_cn}</h2>
                <h1 style='color:#0071e3; margin:15px 0;'>{structured_cmd if current_mode == 'advanced' else current_command}</h1>
                <p style='font-size:14px;'>置信度：{confidence}%</p>
    ...
    # 动态测试生成与结果展示（新增下方框架，不改变现有布局）
    st.markdown("---")
    st.subheader("🧪 动态安全测试生成与结果展示")
    ...
    with dynamic_test_container:
        if st.session_state.dynamic_test_cases and st.session_state.test_results:
            ...
            with gen_col:
                st.markdown("#### 🎲 动态测试生成")
                generated_df = pd.DataFrame(st.session_state.dynamic_test_cases)
                generated_df = generated_df.rename(columns={
                    "name": "测试名称",
                    "category": "覆盖维度",
                    "intent": "意图",
                    "command_id": "指令ID",
                ...
                if "期望" in generated_df.columns:
                    generated_df["期望"] = generated_df["期望"].map(lambda x: "允许" if x else "拦截")
                st.dataframe(generated_df, use_container_width=True, height=320)

            with result_col:
                st.markdown("#### 📋 测试结果单独显示")
                results_df = pd.DataFrame(st.session_state.test_results)
                ...
                if "是否通过" in results_df.columns:
                    results_df["是否通过"] = results_df["是否通过"].map(lambda x: "✅ 通过" if x else "❌ 未通过")
                st.dataframe(results_df, use_container_width=True, height=320)
        else:
            st.info("点击左侧“🧪 运行安全测试”后，这里会在原有界面下方单独显示动态生成的测试内容与执行结果。")
